    ...
    try {
        ...
        if (fulfillmentOrders.data?.fulfillment_orders?.length > 0) {
            ...
            if (firstFulfillmentOrder.assigned_location?.name) {
                // Iterate through each line item to check for matching variant_id
                for (const lineItem of firstFulfillmentOrder.line_items) {
                    if (lineItem.variant_id === variant_id) {
                        return firstFulfillmentOrder.assigned_location.name;
                    }
                }
                // If no matching variant_id found in any line item, still return the location name
                return firstFulfillmentOrder.assigned_location.name;
            }

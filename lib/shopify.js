...
export async function riskOrders(order) {
    try {
        ...
        if (Array.isArray(risks) && risks.length > 0) {
            ...
            console.warn(`No risk data found for order ${order.id}`);
            return null;
        }
    } catch (error) {
        console.error(`Failed to while fetching the risk order ${order.id}:`, error.message);
        return null;
    }
}
...
export async function checkAddressIssue(order) {
    try {
        ...
        console.error(`Failed to fetch validationResultSummary for order ${order.id}:`, error.message);
        return null;
    }
}
